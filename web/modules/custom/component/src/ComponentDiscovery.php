...
class ComponentDiscovery implements ComponentDiscoveryInterface {
  ...
  protected $defaults = [
    ...
    'dependencies' => [],
    // Define the machine name of the component that this will override.
    'parent' => '',
  ];
  ...
  public function getComponents() {
    ...
    // Find component yml files. This provides an array of file paths keyed
    // by the machine_name (filename) of the component.
   ...
   *
   * The information is returned in an associative array, keyed by the
  ...
  protected function parse($filepath) {
    ...
    if (file_exists($filepath)) {
      ...
      // @todo Fix test required keys.
      // @todo Fix right now this kills the site.
      // Validate that the component has the required keys.
      $missing_keys = array_diff($this->required, array_keys($parsed_data));
      if (!empty($missing_keys)) {
        unset($parsed_data);
        $required_keys = implode(', ', $missing_keys);
        $this->logger->notice('Component parsing error: missing required keys @keys in @filepath', [
          '@keys' => $required_keys,
          '@filepath' => $filepath,
        ]);
      }
  ...
  protected function scanDirectory($dir) {
    ...
    // for component ymls. This type of action can be resource intensive.
